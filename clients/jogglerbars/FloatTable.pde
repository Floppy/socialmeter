...
class FloatTable {
  ...
  FloatTable(String filename) {
    ...
    rows = sort(rows); // order rows alphabetically
    ...
    rowOwners = new String[rows.length-1];
    rowImages = new String[rows.length-1];
    ...
    for (int i = 1; i < rows.length; i++) {
      ...
      // copy rowOwner
      rowOwners[rowCount] = pieces[1];
      // copy row Image
      rowImages[rowCount] = pieces[6];
  ...
  String getRowOwner(int rowIndex) {
    return rowOwners[rowIndex];
  }
  ...
  String getRowImage(int rowIndex) {
    return rowImages[rowIndex];
  }
